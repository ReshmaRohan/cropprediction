...
class BaseHighCharts(BaseChart):
    ...
    def get_series(self):
        ...
        for i, name in enumerate(series_names):
            serieses.append({"name": name, "data": column(data, i+1)[1:],"dashStyle": "ShortDash","color": "#90ed7d"})
        return serieses
...
class ColumnLineChart(BaseHighCharts):
    ...
    def get_series(self):
        data = self.get_data()
        serieses = []
        serieses.append({"name": data[0][1], "data": column(data, 1)[1:], "type": "column","color": "#8085e9"})
